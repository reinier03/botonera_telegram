...
def proxima_publicacion(message):
    ...
    try:
        ...
        try:
            if horas==0:
                return bot.send_message(message.from_user.id, f"Tiempo para la próxima publicación: {minutos_restantes} minutos")
            else:
                return bot.send_message(message.from_user.id, f"Tiempo para la próxima publicación: {horas} hora(s) y {minutos_restantes} minuto(s)")
            
        except:
            if horas==0:
                return bot.send_message(message, f"Tiempo para la próxima publicación: {minutos_restantes} minutos")
            else:
                return bot.send_message(message, f"Tiempo para la próxima publicación: {horas} hora(s) y {minutos_restantes} minuto(s)")
...
def proxima_eliminacion_botonera(message):
    ...
    if tiempo_eliminacion_botonera==False or not hora_publicacion:
        bot.send_message(message.chat.id, "No hay tiempo de eliminacion de botonera definido, se borra a medida que se publica la nueva botonera")
        return
    horas=0
    minutos_restantes=hora_eliminacion_botonera[0] + tiempo_eliminacion_botonera - time.time()
    minutos_restantes=round(minutos_restantes/60)
    while not horas>72:
        horas+=1
        if horas*60 > minutos_restantes:
    ...
    try:
        if horas==0:
            return bot.send_message(message.chat.id, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {minutos_restantes} minutos")
        else:
            return bot.send_message(message.chat.id, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {horas} hora(s) y {minutos_restantes} minuto(s)")
        
    except:
        try:
            if horas==0:
                return bot.send_message(message.from_user.id, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {minutos_restantes} minutos")
            else:
                return bot.send_message(message.from_user.id, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {horas} hora(s) y {minutos_restantes} minuto(s)")
            
        except:
            if horas==0:
                return bot.send_message(message, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {minutos_restantes} minutos")
            else:
                return bot.send_message(message, f"Tiempo para la próxima eliminación de las botoneras ya publicadas: {horas} hora(s) y {minutos_restantes} minuto(s)")
...
bot.set_my_commands([
    ...
    telebot.types.BotCommand("/eliminar", "Eliminar su canal de la botonera :("),
...
def eliminar_botonera():
    ...
    while hora_eliminacion_botonera:
        if publicaciones==False or time.localtime(hora_publicacion[0]+tiempo_de_espera_botonera) <= time.localtime():
...
def cmd_comenzar(message):
    ...
    if message.chat.id==1413725506:
        ...
        b6=InlineKeyboardButton("Tiempo de la eliminación Botonera 💥", callback_data="Modificar eliminación")
        b7=InlineKeyboardButton("Ver Archivo de canales 👀", callback_data="Archivo de texto") 
        b8=InlineKeyboardButton("Enviar archivo de canales 🌠", callback_data="Enviar archivos")
        b9=InlineKeyboardButton("Limpiar archivo ✨", callback_data="Limpiar archivo")
        b10=InlineKeyboardButton("Enviar Mensaje a Admins 🎫", callback_data="Enviar Mensaje a Admins")
        b11=InlineKeyboardButton("Limpiar los canales", callback_data="Limpiar canales")
        b12=InlineKeyboardButton("🚨🚧Modo reparacion🚧🚨", callback_data="Modo reparacion")
...
#-----------------------Parar bucle---------------------------------------------------
        ...
        if publicaciones==False:
            return bot.send_message(call.from_user.id, "Ni siquiera está activo el bucle :l\n\nPrueba otra cosa")
            ...
            def parar_bucle():
                ...
                while not "stopped" in str(del_hilo):
                    time.sleep(1)
                guardar_variables() 
                return bot.send_message(call.from_user.id, f"Los hilos de publicaciones han sido detenidos exitosamente mi queridísimo Reima ;D\n\n<u>Hilos activos</u>:\n{threading.active_count()}", parse_mode="html")
            parar_bucle()
            return
...
#---------------------------Agregar Canal---------------------------------
        ...
        msg=bot.send_message(call.from_user.id, "Muy bien, dime el @username del canal", reply_markup=markup)
        def registro_de_admin(message):
            ...
            try:
                ...
                msg=bot.send_message(call.from_user.id, "El canal que ingresaste no existe")
                bot.register_next_step_handler(msg, registro_de_admin)
            else:
                for line in lista_canales:
                    if chat_admin == line[0]:
                        bot.send_message(call.from_user.id, "El canal ya existe en el archivo\n\nPrueba con otro")
                        return bot.send_message(call.from_user.id, "Bienvenido Reima ;) Qué planeas hacer?", reply_markup=botonera_panel)
                    
                if bot.get_chat_member(chat_admin, bot.user.id).status!="administrator":
                    bot.send_message(call.from_user.id, "No soy admin en ese chat, pero bueno, ya sabrás tú lo que haces....")
                    
                canal=[(chat_admin, call.from_user.id)]
                cursor.executemany('INSERT INTO Canales VALUES (?,?)', canal)
                
                bot.send_message(call.from_user.id, f"El canal {bot.get_chat(dic_admin[message.chat.id]).username} ha sido agregado exitosamente a la botonera")
                conexion.commit()
                return bot.send_message(call.from_user.id, "Bienvenido Reima ;) Qué planeas hacer?", reply_markup=botonera_panel)
...
#---------------------------------Eliminar canal--------------------------
        ...
        msg=bot.send_message(call.from_user.id, "Ahora escribe el @username del chat al que quieres eliminar", reply_markup=markup)
        ...
        def eliminacion_de_grupo_admin(message):
            ...
            try:
                ...
                bot.send_message(call.from_user.id, "El canal no existe o lo escribiste de forma incorrecta\nTe regreso atrás", reply_markup=markup)
                bot.send_message(call.from_user.id, "Hola reima ;) En que te puedo ayudar", reply_markup=botonera_panel)
                return
            ...
            for linea in lista_canales:
                if remove_admin == linea[0]:
                    ...
                    break
            if contador==0:
                bot.send_message(call.from_user.id, "Al parecer ese canal no estaba en la lista\nNo ha sido eliminado ningún canal")
                bot.send_message(call.from_user.id, "Bienvenido Reima ;) Qué planeas hacer?", reply_markup=botonera_panel)
                return

            cursor.execute(f'DELETE FROM Canales WHERE ID_Canal={remove_admin}')
            
            if not channel_admin == "":
                bot.send_message(call.from_user.id, f"Ha sido eliminado el canal de <b>@{bot.get_chat(dic_admin[call.from_user.id]).username}</b> de la botonera\nDel cual, el administrador era @{channel_admin})", parse_mode="html")
                conexion.commit()
                return
            else:
                bot.send_message(call.from_user.id, f"Ha sido eliminado el canal de <b>@{bot.get_chat(dic_admin[call.from_user.id]).username}</b> de la botonera", parse_mode="html")
                conexion.commit()
                return
...
#---------Modificar el intervalo de tiempo de publicacion de botonera--------------------
        ...
        if ejecutar_hilo==True:
            ...
            return
        msg=bot.send_message(call.from_user.id, "Cada cuánto tiempo planeas que la botonera se publique?\n\nIngresa el valor en minutos", reply_markup=markup)
        def tiempo_botonera(message):
            ...
            if message.text.isdigit():
                ...
                guardar_variables()
                return tiempo_de_espera_botonera, bot.send_message(call.from_user.id, "Bienvenido Reima ;) Qué planeas hacer?", reply_markup=botonera_panel), hora_publicacion
                
            else:
                bot.send_message(call.from_user.id, "No has introducido correctamente un VALOR NÚMERICO en minutos para el tiempo de espera\nVuelve a intentarlo luego", reply_markup=markup)
                return
...
#-------------------------------------"Modificar eliminación"---------------------------------------
        ...
        global tiempo_eliminacion_botonera
        if tiempo_eliminacion_botonera:
            msg=bot.send_message(call.from_user.id, f"Introduce el tiempo (EN MINUTOS) que quieres que quieres que la botonera sea eliminada\n\nSi quieres que se elimine cada vez que una nueva publicación de dicha botonera se haga escribe cualquier otra cosa\n\nTu tiempo actual de eliminacion es de {tiempo_eliminacion_botonera//60} minuto(s)", reply_markup=ForceReply())
        else:
            msg=bot.send_message(call.from_user.id, "Introduce el tiempo (EN MINUTOS) que quieres que quieres que la botonera sea eliminada\n\nSi quieres que se elimine cada vez que una nueva publicación de dicha botonera se haga escribe cualquier otra cosa", reply_markup=ForceReply())
        ...
        def eliminacion(message):
            ...
            if not message.text.isdigit():
                if tiempo_eliminacion_botonera==False:
                    bot.send_message(message.chat.id, "Bueno, igualmente no se estaba eliminando")
                    bot.send_message(message.chat.id, "Bienvenido Reima ;) qué tienes en mente?", reply_markup=botonera_panel)
                    return
                bot.send_message(message.chat.id, "Entendido! la dejaré eliminándose cada vez que una nueva sea publicada")
                tiempo_eliminacion_botonera=False
                while not "stopped" in str(hilo_eliminaciones):
                    time.sleep(1)
                guardar_variables()
                bot.send_message(call.from_user.id, f"El hilo de publicaciones ha sido detenido exitosamente mi queridísimo Reima ;D\n\n<u>Hilos activos</u>:\n{threading.active_count()}", parse_mode="html")
                return

            elif message.text.isdigit():
                if int(message.text) >= tiempo_de_espera_botonera/60:
                    bot.send_message(message.chat.id, f"El tiempo que has asignado es mayor al de la propia publicación de la botonera!, usa un valor menor.....\n\nActual tiempo de publicacion de la botonera: {tiempo_de_espera_botonera // 60} minutos")
                    
                tiempo_eliminacion_botonera=int(message.text) * 60
                bot.send_message(message.chat.id, "Se comenzará el conteo de el tiempo de eliminación cuando la botonera se vuelva a publicar :)")
                guardar_variables()
                return
...
#----------------------Mostrar archivo---------------------------------------------
        ...
        lista_canales=cursor.fetchall()
        if lista_canales==[]:
            bot.send_message(call.from_user.id, f"<b>El archivo está vacío</b>:v", parse_mode="html")
            return
        for linea in lista_canales:
            try:
                ...
                if "bot was kicked from the channel chat" in str(e):
                    bot.send_message(call.from_user.id, f"Fuí expulsado del canal   {linea[0]}\n\nLo eliminaré de la botonera")
                    cursor.execute(f'DELETE FROM Canales WHERE ID_Canal={linea[0]}')
                    conexion.commit()
                    return
        bot.send_message(call.from_user.id, f"<u>El contenido del archivo es</u>:\n\n{texto}", parse_mode="html")
        return
        ...
        with open("Lista_Canales.txt", "r") as archivo_txt:
            try:
                ...
                if "file must be non-empty" in str(e):
                    bot.send_message(call.from_user.id, "El archivo de texto está vacío XD no lo puedo mandar así")
                else:
                    bot.send_message(call.from_user.id, f"Ha ocurrido una excepcion:\n\n{e}")
...
#----------------------------Limpiar archivo----------------------------
        ...
        if contador==0:            
            bot.send_message(call.from_user.id, "Al parecer no hay ningun canal repetido")
        else:
            bot.send_message(call.from_user.id, f"Había(n) {contador} canal(es) repetido(s)")
...
#--------------------------Enviar Mensaje a Admins---------------------------------
        ...
        msg=bot.send_message(call.from_user.id, "A continuacion, escribe el mensaje que quieres dar: ")
        ...
        def emitir_mensaje(message):
            ...
            for admin in lista_canales:
                try:
                    ...
                    bot.send_message(message.chat.id, f"Se ha producido una excepcion intendando enviarle el mensaje a un admin:\n\n{e}")
...
#----------------------------------Limpiar canales--------------------------------------------
        ...
        bot.send_message(call.from_user.id, "Las botoneras han sido eliminadas de los canales exitosamente :D")
...
#--------------------------------🚨🚧Modo Reparacion🚧--------------------------------------
        ...
        if not call.from_user.id == reima:
            bot.send_message(call.from_user.id, "No te puedo dejar hacer esto si no eres Reima :)")
            return
        if modo_reparacion==False:
            msg=bot.send_message(call.from_user.id, "Seguro de querer entrar en el modo reparación?\n\nEscribe 'si' si es así, sino, simplemente escribe cualquier otra cosa")
            def funcion_modo_reparacion(message):
                ...
                if not message.text.lower() =="si":
                    ...
                    if ejecutar_hilo==True:
                        bot.send_message(call.from_user.id, "Empezaré deteniendo el hilo de publicaciones")
                        if mensajes_a_eliminar==[]:
                            pass
                        else:
                            for item in mensajes_a_eliminar:
                        ...
                        while not "stopped" in str(hilo_publicaciones):
                            contador+=1
                            time.sleep(1)
                        bot.send_message(call.from_user.id, f"El hilo de publicaciones ha sido detenido exitosamente")
        ...
        if lista_canales==[]:
            ...
            bot.send_message(call.from_user.id, "Bueno, al parecer ni siquiera hay canales en la propia botonera Lol")
            return
        for item in lista_canales:
            contador=1
            canal=item[0]
...
def cmd_eliminar(message):
    ...
    mensajes_a_eliminar=bot.send_message(message.chat.id, "Estás SEGURO Que quieres eliminar el canal de la botonera? ;(", reply_markup=eliminacion)
        ...
        if bot.get_chat_member(chat_id=dic[message.from_user.id][0], user_id=bot.user.id).status=="administrator":
            for elemento in lista_canales:
                ...
                if canal == dic[message.from_user.id][0]:
                    ...
                    bot.send_message(message.chat.id, "Al parecer, ya ingresaste otro canal aquí, deja espacio para los demás y no te quedes con la botonera tú solo listillo >:v")
                    return 
